...
3. If a grilling session has already been conducted in this conversation and its wrap-up covered the decision space needed for the PRD, skip this step. Otherwise, read and run a grilling skill on the plan: prefer `grill-with-docs` if it's available (it adds inline `CONTEXT.md` updates and ADR offers on top of the same interview engine), and fall back to `grill-me` if not. If neither skill is installed, skip this step.
