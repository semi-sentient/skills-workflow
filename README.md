...
| `grill-with-docs` | Stress-test a plan against the project's existing domain model and documented decisions, sharpening terminology and updating `CONTEXT.md` / ADRs inline as decisions crystallise.                                                                                                                                                                                                                                                |
...
| `run-plan`        | Orchestrate a multi-phase implementation plan by delegating phases to specialized sub-agents (Research, Code, Architect, Debug, Review) with fresh context windows. An independent review gate audits each Code phase before its commit, plus a branch-scope review before the PR. Accepts a plan file path, a `#N` plan-sub-issue reference, or a full issue URL. Owns the full git/GH ceremony (work branch, per-phase commits, sub-issue sync, PR submission) with opt-out flags (`--no-branch`, `--no-github`, `--no-pr`, `--no-review`, `--no-branch-review`). |
| `commit`          | Generate a Conventional Commits message from staged changes and commit. Accepts an optional ticket identifier; when omitted, infers an issue reference from the branch name or conversation — never blocking a commit on one.                                                                                                                                                                                                                                                                                                          |
| `tend-steering-docs` | Condense or restructure a repo's agent steering docs (`AGENTS.md`, `CLAUDE.md`, `docs/agents/*`) without losing load-bearing rules. Surfaces contradictions between rules and gates every removal behind a human-reviewed Removal Ledger. Scope is steering docs only — not READMEs, ADRs, or `CONTEXT.md`.                                                                                                              |
...
| ------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `run-plan`    | `commit`                                 | Per-phase commit messages are authored against the `commit` skill — by each Code agent on the fast path, or by invoking `/commit` directly as the fallback. Without it, the commit step fails.                                                                                   |
...
| `tend-steering-docs` | `grill-with-docs` or `grill-me` _(soft)_ | Optionally invokes a grilling skill to make the agent defend each proposed cut in the Removal Ledger against the gauntlet. Works fully standalone without it; the grill step is skipped if neither is installed. |
