...
- **User stories covered**: which user stories from the PRD this addresses (a PRD organized as numbered decisions instead of stories: cite the decision numbers, and PRD coverage maps decisions to phases)
...
Before writing the file, verify the plan against the **actual codebase**, not your memory of it. Reviewers need a document to read, so first render the full plan (Step 7d's template) to a **scratch draft** outside the plans directory — resolve a scratch location at runtime, checking in this order: a scratch directory the repo's own instructions mandate, then the host's scratch or temp directory, then a `scratch/` sibling of the plans directory ( when the location is inside the repo, verify git ignores it with `git check-ignore` and add it to `.git/info/exclude` yourself when it doesn't — never assume coverage), never the plans directory itself, which receives only the approved artifact at Step 7 — and hand reviewers that path, telling them in the brief that the scratch location is deliberate and not a finding. Every fix in this step edits the draft; Step 7 copies the approved draft into place. This catches the #1 source of plan errors: working from stale memory instead of real files. The agent that drafted the plan reads it charitably — it knows what each phase _meant_ — so wherever the host allows, this verification should be done by an **independent reviewer with a fresh context window**, not by the author re-reading its own work.
...
- **Criteria verifiability** — Every acceptance criterion must be checkable by `run-plan`'s Review agent from the repository alone. Three sub-checks: (1) **Executable**: run every read-only shell command a criterion states, from the directory the criterion names, and report what it returns today against what the plan says it must return after the phase — a command that already passes proves nothing, one that cannot pass on a correct implementation will make the executing agent change working code, and one that is invalid on the host's shell (BSD vs GNU `grep`/`sed`/`regcomp` — `\s` is not portable) is silence indistinguishable from a pass. (2) **Evidence location**: a criterion that requires recorded output names a location the reviewer can read — the phase's commit body, a committed file, or the synced plan issue — never "the progress note", "the summary", or the conversation: those are not durable and the reviewer cannot see them. (3) **Actor**: a criterion only a human can satisfy (an apply, a console check, a live-traffic observation) begins with `Human verifies …` / `Human confirms …`, so the executing skill's human-gate rule identifies it mechanically; in a phase that carries a `**Known risk**: human action` line, EVERY such criterion is labelled, not just one.
...
The irreducible-risk classification is only available for a phase with zero outstanding Step 6b findings — specification vagueness generally surfaces as a 6b finding, so a clean checklist is the operational test for "nothing fixable remains" (the counterfactual score below backstops what 6b misses). The reviewer must also report the counterfactual score the phase would earn if the named cause were resolved: the exception applies only if that counterfactual is 9+ and the actual score is 5 or higher. Below 5, the irreducible-risk exception is unavailable — return to Step 5 and split or checkpoint the risky work into its own thin phase instead (this floor governs only the exception: a below-5 phase classified as a specification gap is fixed per the next paragraph and never split on its score alone); a phase that is already such a minimal risk-isolating slice with a human checkpoint in its acceptance criteria is exempt from this floor (otherwise splitting would loop forever). In fallback mode the classification, like the score, is the author's own claim; mark such rows as self-classified in the Step 6d table so the user's approval is informed.

Flag any phase below 9. Fix every Step 6b checklist issue before proceeding. For a flagged phase classified as a specification gap, add concrete implementation details (code snippets, API configurations, exact function signatures) until the score reaches 9+ or the remaining shortfall is entirely a named cause. After applying fixes to any flagged phase, re-run verification in the same mode as Step 6a; the scores and classifications presented in Step 6d must come from that re-run — in independent mode never from the author, in fallback mode from a fresh red-team pass rather than carried over from before the fixes. A re-run may be scoped to the phases the fixes touched (plus any phase whose cross-phase references changed), or to a named fix set or single finding when the fixes cut across phases — either way the brief states the scope; the Step 6d table then labels every score with the round that produced it, so an unchanged phase's older score is visibly older and never presented as current.

**Convergence.** Every round's reviewer brief — round 1 included — asks the reviewer to class each finding as **design** (any Step 6b item except Criteria verifiability) or **criteria** (Criteria verifiability), and to grade each design finding **critical** (the phase as written builds the wrong thing or cannot build), **major** (an executing agent would stall or guess), or **minor** (anything else). A round whose design findings include no critical and no major has converged, whatever the scores: fix its remaining findings, re-run Criteria verifiability alone on the changed criteria, and proceed to Step 6d — the table discloses the post-convergence fixes, and a phase still below 9 is presented with its score and the note "no major design findings outstanding"; the user decides whether to publish. Zero findings is not the bar: an adversarial reviewer returns something every round, and a fix round can introduce the next round's findings — all-minor is the exit, not silence. A round that does NOT converge routes through the normal loop: the author fixes its findings and a reviewer re-runs per Step 6c, whose scoping rule applies unchanged; convergence decides only when the design re-run is skipped, never who runs what. An author-adjusted score is never presented: the Step 6d table carries the last reviewer-assigned score for each phase, labelled with its round, however many author fixes landed after it. A finding that flags an existing criterion as defective is answered by rewriting or deleting that criterion, never by adding another beside it; a finding that a needed criterion is missing is a coverage gap and legitimately adds one. Criteria added during verification are how a phase crosses the split signal and restarts the loop — in production, two runs each spent their last two of six rounds this way. Never pad a phase with invented detail to inflate its score; fabricated snippets read as authoritative and are worse than a flagged phase.
...
**Step 6d — Present verification results** — Show the user a summary table with each phase's confidence score (0–10), any issues found, and the changes made to resolve them. If verification changed the structure the user approved at Step 5 (a phase merged or split), the table presents the new structure and states what changed and why — the Step 5 approval is not re-run; this approval covers it. For any phase presented below 9 under the irreducible-risk exception, the table must state the named cause (and mark it self-classified in fallback mode) so the user is accepting that risk explicitly, not overlooking it. Do NOT proceed to Step 7 until the user explicitly approves. If the user requests changes, revise, re-verify (same mode as above), and re-present. Repeat until approved.
...
- State the resulting filename in the Step 6d approval message; the user's approval covers it (no separate confirmation)
