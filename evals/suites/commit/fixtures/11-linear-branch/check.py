...
def check(ctx, expect):
    ...
    # SKILL.md step 2.2 — lowercase key in a non-first segment is captured and
    # uppercased on output; inferred tickets get the scope like supplied ones.
    expect.match("subject carries the uppercased Linear key as a scope",
                 rf"^({TYPE_RE})\(ENG-142\): [A-Z]", subj)
    expect_explanatory_body(expect, msg)

    lines = [l for l in msg.splitlines() if l.strip()]
    expect.that("Ticket: footer is the final line",
                bool(lines) and lines[-1].strip() == "Ticket: ENG-142",
