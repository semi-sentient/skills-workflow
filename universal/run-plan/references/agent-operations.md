...
Read this when SKILL.md points here: to compose an Architect or Debug brief (the four templated briefs need nothing from this file), when a scoped re-review looks applicable (Scoped Re-review Exceptions), or at Step 5c.5 for the pre-PR Review variant. It is not held in working memory; the brief templates under `briefs/` are the definitions the agents actually receive, and `rp.sh brief` fills them.
...
## Pre-PR Review variant (SKILL.md Step 5c.5)

Compose inline to `<scratch_dir>/pre-pr-brief-review.md` from `brief-review.md`'s role paragraph, substituting: scope = the branch diff, `git diff <base_branch>...HEAD`, or `git diff <inputs_commit_sha>...HEAD` when Step 5c.5 selects it (that step owns the selection rule; a mid-branch inputs commit keeps the `<base_branch>` ref and is named in the brief as out of scope instead); mandate = correctness bugs, with emphasis on integration seams between phases and on forward-compatibility hooks from phase summaries (list them in the brief) that later phases should have resolved; output = surviving findings only, each verified against the code before reporting, with a CONFIRMED/PLAUSIBLE tag per finding, and no evidence file. On Claude Code, instruct the sub-agent to invoke the installed `code-review` skill (effort medium) if available — it owns review methodology, including adversarial verification of findings; the generic mandate is the fallback for hosts without it.
