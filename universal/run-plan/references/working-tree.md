...
**Omit `<plan_file_path>` from the list.** The plan file is the run's own record: Step 4 item 6 edits it and item 7 commits it every phase, so it can never be keep-dirty — excluding it would strand every checkbox edit outside the commit record and arm the next resume's interrupted-phase correction (branch-and-resume.md) against genuinely completed phases. Pre-existing dirt in it simply rides the run's normal staging. If the omissions leave nothing to list, record nothing and return to SKILL.md Step 1e.3 — there is no one to ask about an empty list.
...
2. `git diff --cached --quiet -- <precommit_pathspec>` — **scoped to the allowlist**. If it reports nothing staged, confirm why before continuing: when those paths are absent from `git status` they were already committed by a prior run, so clear `<precommit_pathspec>`, delete their `input:` lines from `tree-state.md`, and continue (SKILL.md Step 2's reporting item names the prior run's commit); if they are still dirty, item 1 silently failed — stop and surface it, because continuing leaves them for Phase 1's staging to swallow.
3. `Skill(skill="commit", args="#<plan_sub_issue_number>")` (pass `args="--no-ticket"` in local-only mode, so the skill doesn't infer a ticket this run deliberately has none of). A sanctioned inline diff read, like Step 4 item 7's fallback.
